...
# parametri/costanti
scala_lavoratori_capital_good_parametro_mail_roventini = 4 # Con 1 è come se non ci fosse
parametro_mail_roventini_scala_lavoratori = 4 # questa è la parte costante, sarà moltiplicata per la produttività media, di chi? Forse delle consumption
...
econometric_sample_size_T = 300-150
periodi_durata_simulazione_turni = 4 * econometric_sample_size_T
# periodi_durata_simulazione_turni = 50 #200
consumption_da_seguire_e_stampare = 10
#%%
# INIZIALIZZAZIONE
...
# QUANDO LE AZIENDE CONSUMPTION j SCEGLIERANNO LE MACCHINE CI VUOLE UN if CHE SALTA LA PROCEDURA SE NON HANNO RICEVUTO NEMMENO UNA BROCHURE E QUINDI NON ORDINANO NIENTE E SE NON COMPARIRANNO TRA I NC_i DI QUALCHE CAPITAL LE LORO MACCHINE CHE SUPERANO eta SARANNO SCARTATE E NON POTRANNO ESSERE RIMPIAZZATE !!!
...
# D_j_domanda_turno_precedente = np.round( (1+alpha_trigger_rule) * K_j_0_capital_stock_iniziale_per_azienda_beni * average_labor_productivity_iniziale * (1/(1+theta_parameter_desired_inventories)) * np.ones(F1_numero_consumption_industry) ) # SENZA u QUINDI 100% INVECE DI 75%
...
# per me questo è sbagliato poi Eliminare perchè sono soldi per cui non può essere un intero
...
for turno in range(periodi_durata_simulazione_turni):
    ...
    if turno == 0:
        ...
        AB_di_questo_turno_appena_calcolata = ( np.sum( np.sum(aziende_consumption_good[falso]) for falso in range(F1_numero_consumption_industry) ) )/( np.sum(len(fake) for fake in aziende_consumption_good) ) # variante: caso media solo consumption
    ...
    for azienda_j_consumption in range(F1_numero_consumption_industry):
        ...
        if restart_j_t_fallite_tutte == 1:
            ...
            # qua metto quelle parti che sarebbe logico aspettarsi di trovarle nella parte molto che sta molto più sotto "Entry and Exit" delle consumption dove c'è cosa fare se sono tutte morte e i comandi per il restart.
            ...
            # D_j_domanda_turno_precedente[azienda_j_consumption] = int( np.around( ( (1/F1_numero_consumption_industry) * w_salario_questo_turno * (parametro_percentuale_forza_lavoro*L_0_labor_supply_popolazione_lavoratori) ) /( (1 + mi_0_mark_up_iniziale)*(w_salario_questo_turno/AB_di_questo_turno_appena_calcolata) ) ) ) # siccome sopra nell'inizializzazione l'array è stato creato np.int64 rimarrà tale anche se usi "int"
        ...
        # Forse come condizione iniziale, visto che non è riportata nel paper, si potrebbe fare che sia uguale alla quantità prodotta che è capital_stock * F1_numero_consumption_industry = 800*200 e che la merce invenduta è zero: magazzini vuoti. Oppure un percentuale di occupazione per il salario divisa per il prezzo iniziale di un bene di consumo 
        ...
        if rapporto_debito_vendite_j_turno_precedente < lambda_max_debt_sale_ratio:            
            ...
            if (costo_totale_della_produzione_c_j_t_Q_j_t - NW_j_t_liquid_assets_consumption[azienda_j_consumption]) > quanto_debito_si_puo_ancora_fare:
                ...
                # Inizialmente avevo scritto però per me è sbagliato: quantità_Q_permessa = c_j_t_unit_cost_production * produzione_permessa
                ...
                RI_j_t_replacement_investimenti = 0 # però così i macchinari piu' vecchi di eta saranno buttati via e non sostituiti e quindi il # delle A in Theta_j calerà
                                ...
                                for indice_A_posseduta_appartenente_theta_j_t in range(K_j_t_macchine_turno_precedente):
                                    ...
                                    if eta_macchinari_vecchiaia_consumption_aziende[azienda_j_consumption][indice_A_posseduta_appartenente_theta_j_t] >= (eta_max_machine_age-2):
                                        ...
                                        # nessun bisogno di cambiare il segno della disequazione poichè stai moltiplicando per un numero positivo. Invece (1/B) <= b(1/A - 1/A_new) funziona sempre, anche in caso di segno negativo se A>A_n infatti +<- fornisce "False" e quindi l'if viene saltato non essendo "True".
                                            ...
                                            # if (scala_lavoratori_capital_good_parametro_mail_roventini/aziende_capital_good_B_i_tau[indice_azienda_capital_brochure]) <= b_payback_period * ( (1/A_posseduta_appartenente_theta_j_t) - (1/aziende_capital_good_A_i_tau[indice_azienda_capital_brochure]) ):
    ...
    for azienda_i_capital in range(F2_numero_capital_industry):
        ...
        #if Q_i_t_quante_A_macchine_deve_produrre < aziende_capital_good_B_i_tau[azienda_i_capital]:
        # secondo me come avevi scritto sopra è sbagliato poichè se Q_i_t=0 poichè nessuna _j ha ordinato macchinari allora per forza questa sarà minore di B_i ma _i assume 1 lavoratore anche se non doveva produrre!
    ...
    # Però comunque con np.around ti ritrovi un vettore con la virgola anche se senza decimali (però arrotondato correttamente) siccome il vettore rimane di tipo float, ad esempio così: array([4., 2., 1.]). Quindi poi lo devi convertire a int di np.array col comando np.int_ togliendo così la virgola cioè il punto .
    for azienda_j_consumption in range(F1_numero_consumption_industry):
        if D_j_t_domanda_questo_turno_quantita[azienda_j_consumption] >= Q_j_t_quantita_prodotta_nel_turno_attuale[azienda_j_consumption]:
            ...
            # N_j_turno_precedente_merce_invenduta[azienda_j_consumption] = N_j_turno_precedente_merce_invenduta[azienda_j_consumption] + quanto_in_piu_prodotto
    ...
    if turno > 0:
        ...
        soglia_sotto_alla_quale_consumption_j_muore = (1/F1_numero_consumption_industry)*(1/3) # PER FAR OSCILLARE DI PIU' I CONSUMI CREANDO CRISI
        ...
        if len(lista_estrazione_indici_j_fallite) > 0:
            ...
            if len(lista_estrazione_indici_j_non_fallite) > 0:
                ...
                for azienda_fallita_j_consumption in lista_estrazione_indici_j_fallite:
                    ...
                    # QUESTA RIGA SOTTO ERA ATTIVA!!!111!11111 L'HAI TOLTA PER PROVARE SE RIESCI AD ALLINEARE LE _j CON LA REGOLA DEGLI RI ogni b ANNI:
                    ...
                    # Q_j_quantita_prodotta_turno_precedente[azienda_fallita_j_consumption] = np.round( (1+alpha_trigger_rule) * len(aziende_consumption_good[indice_estratto_consumption_da_clonare]) * (np.sum(aziende_consumption_good[indice_estratto_consumption_da_clonare])/len(aziende_consumption_good[indice_estratto_consumption_da_clonare])) * (1/(1+theta_parameter_desired_inventories)) ) # SENZA u QUINDI 100% INVECE DI 75%
                    # D_j_domanda_turno_precedente[azienda_fallita_j_consumption] = np.round( (1+alpha_trigger_rule) * len(aziende_consumption_good[indice_estratto_consumption_da_clonare]) * (np.sum(aziende_consumption_good[indice_estratto_consumption_da_clonare])/len(aziende_consumption_good[indice_estratto_consumption_da_clonare])) * (1/(1+theta_parameter_desired_inventories)) ) # SENZA u QUINDI 100% INVECE DI 75%
        ...
        if len(lista_estrazione_indici_i_fallite) > 0:
            ...
            if len(lista_estrazione_indici_i_non_fallite) > 0:
                ...
                for azienda_fallita_i_capital in lista_estrazione_indici_i_fallite:
                    ...
                    NW_i_t_liquid_assets_capital[azienda_fallita_i_capital] = scala_lavoratori_capital_good_parametro_mail_roventini * NW_i_j_0_liquid_assets * w_salario_questo_turno # se il Pil va a zero o si riduce sensibilmente poi le nuove capital avranno NW_i quasi nulli
...
plt.grid()
plt.ylim(0)
plt.savefig('K+S EI e RI log.jpg', dpi=650, transparent=False)
plt.show()
...
plt.plot(t_time_turno, quante_fallite_j_aziende_questo_turno_storico_tutti_turni/F1_numero_consumption_industry,'-',color='black',label='consumption $j(t)$')
plt.plot(t_time_turno, quante_fallite_i_aziende_questo_turno_storico_tutti_turni/F2_numero_capital_industry,'-',color='blue',label='capital $i(t)$')
plt.legend(loc='best',prop={'size': 6})
#plt.legend(loc='lower right', prop={'size': 6})
...
# plt.ylabel('$\omega, \lambda$')
plt.grid()
plt.savefig('K+S occupazione.jpg', dpi=650, transparent=False)
plt.show()
...
plt.plot(t_time_turno, np.log(I_t_senza_zeri_valore_monetario_investment_consumption_firms_storico_tutti_turni),'--b',label='Inv. $I(t)$')
plt.plot(t_time_turno, np.log(C_senza_zeri_consumi_soldi_quantita_x_prezzi_storico_tutti_turni),'-.',color='black',label='Cons. $C(t)$')
plt.legend(loc='best',prop={'size': 6})
plt.xlabel('Time t')
plt.ylabel('log')
plt.grid()
plt.ylim(0)
plt.savefig('K+S soldi quantitàxprezzi log solo I e C.jpg', dpi=650, transparent=False)
plt.show()
...
plt.grid()
plt.ylim(0)
plt.savefig('K+S soldi quantitàxprezzi log solo Y.jpg', dpi=650, transparent=False)
plt.show()
...
#plt.plot(t_time_turno, D_N_variazione_magazzini_soldi_quantita_x_prezzi_storico_tutti_turni,'-',c='hotpink',label='Var. Magaz. $\Delta N(t)$')
#plt.legend(loc='best',prop={'size': 6})
#plt.title('Negative variazione dei magazzini',fontdict = font)
#plt.xlabel('Time t')
#plt.ylabel('NON SONO log')
#plt.grid()
#plt.ylim(10,-10000)
# plt.savefig('K+S variazione negativa dei magazzini non log.jpg', dpi=650, transparent=False)
#plt.show()


plt.plot(t_time_turno, np.log(D_N_variazione_magazzini_soldi_quantita_x_prezzi_storico_tutti_turni),'o',c='hotpink',label='Var. Magaz. $\Delta N(t)$')
plt.legend(loc='best',prop={'size': 6})
#plt.title('log dei positivi variazione dei magazzini',fontdict = font)
plt.xlabel('Time t')
plt.ylabel('log')
plt.grid()
plt.savefig('K+S variazione positiva dei magazzini log.jpg', dpi=650, transparent=False)
plt.show()


plt.plot(t_time_turno, np.log( - D_N_variazione_magazzini_soldi_quantita_x_prezzi_storico_tutti_turni),'o',c='hotpink',label='Var. Magaz. $\Delta N(t)$')
plt.legend(loc='best',prop={'size': 6})
#plt.title('log dei negativi resi positivi variazione dei magazzini',fontdict = font)
plt.xlabel('Time t')
plt.ylabel('log')
plt.grid()
plt.savefig('K+S variazione negativa resa positiva dei magazzini log.jpg', dpi=650, transparent=False)
plt.show()
