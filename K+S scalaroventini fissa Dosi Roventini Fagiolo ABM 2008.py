...
# parametri/costanti
scala_lavoratori_capital_good_parametro_mail_roventini = 4 #1 # Con 1 è come se non ci fosse
...
# Prova per non fare partire tutte le aziende sincronizzate con la stessa domanda prevista ma invece sfasate!
...
S_j_t_attuale_total_sales_soldi = np.zeros(F1_numero_consumption_industry) # FORSE NON E' NECESSARIO FORSE CANCELLARE  # non è uno storico con una lista che contiene un np.array per ogni turno, ma è un unico np.array che contiene solo il turno ATTUALE e deve essere sovrascritto ogni turno
...
for turno in range(periodi_durata_simulazione_turni):
    ...
    for azienda_j_consumption in range(F1_numero_consumption_industry):
        ...
        if Q_desid_j_t < Q_j_t_max_possibile_con_Theta_attuale:
            Q_j_t_nel_paper_non_dice_come = np.round(Q_desid_j_t) # siccome attenzione: non è detto che Q_desid_j_t sia un intero
        else:
        ...
        if rapporto_debito_vendite_j_turno_precedente < lambda_max_debt_sale_ratio:            
            ...
            if (costo_totale_della_produzione_c_j_t_Q_j_t - NW_j_t_liquid_assets_consumption[azienda_j_consumption]) > quanto_debito_si_puo_ancora_fare:
                ...
                #NW_j_t_liquid_assets_consumption[azienda_j_consumption] = 0 # devi consumare tutto il tuo NW (il cash), prima di indebitarti per il restante
                # NW_j_t_liquid_assets_consumption[azienda_j_consumption] = NW_j_t_liquid_assets_consumption[azienda_j_consumption] - costo_totale_della_produzione_c_j_t_Q_j_t # Questo dava il bug con valori NW<0
...
#==============================================================================
# Inizio parte da cancellare una volta che sei sicuro che funziona dal bug
#==============================================================================
...
#==============================================================================
# Fine parte da cancellare una volta che sei sicuro che funziona dal bug
#==============================================================================
    ...
    for azienda_i_capital in range(F2_numero_capital_industry):
        ...
        # prova se idea che i problemi sono dati da INT quando produttività diventa grande è giusta 
    ...
    if int(np.sum(numero_macchine_ogni_capital_i_deve_produrre_per_ordini_consumption)) != 0:
        ...
        f_market_share_tutte_capital_firms = np.zeros(F2_numero_capital_industry) # in questo modo invece è possibile il restart
    if turno > numero_turni_sopra_i_quali_ha_senso_quota_mercato:
        ...
        #soglia_sotto_alla_quale_capital_i_muore = 1/(F2_numero_capital_industry * 10)
